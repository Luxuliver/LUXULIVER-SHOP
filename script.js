...
function updateShopStatus() {
    ...
    const publicHolidays = [
        ...
        '2025-06-09', '2025-06-27', '2025-08-17', '2025-09-05', '2025-12-25', '2025-12-26',

        '2026-01-01', '2026-01-16', '2026-02-17', '2026-03-19', '2026-03-20', 
        '2026-03-21', '2026-04-03', '2026-05-01', '2026-05-14', '2026-05-27', 
        '2026-05-31', '2026-06-01', '2026-06-17', '2026-08-17', '2026-08-25', '2026-12-25'
    ];
    ...
    let isClosed = false;
    let message = "Maaf, toko sedang tutup. Silakan berbelanja lagi nanti!";

    if (publicHolidays.includes(todayString)) {
        isClosed = true;
        message = "Selamat hari libur! Toko sedang tutup dan akan buka kembali besok.";
    } 

    else if (day === 0) {
        isClosed = true;
        message = "Toko tutup pada hari Minggu. Buka kembali besok!";
    } 
    else if (day >= 1 && day <= 5) {
        if (hour < 7 || hour >= 18) {
            isClosed = true;
            message = "Jam operasional Senin-Jumat adalah 07:00 - 18:00. Toko akan buka kembali nanti.";
        }
    } 
    else if (day === 6) { 
        if (hour < 7 || hour >= 20) {
            isClosed = true;
            message = "Jam operasional Sabtu adalah 07:00 - 20:00. Toko akan buka kembali nanti.";
        }
    }

    if (isClosed) {
        statusBanner.querySelector('p').textContent = message;
...
function animateSearchPlaceholder() {
    ...
    const words = ['Velluto', 'Vanderlux', 'The-Beatles', 'Skeleton', 'Travis Scott'];
    const baseText = 'Cari produk: ';
    let wordIndex = 0;
    let charIndex = 0;
    ...
    function typeEffect() {
        ...
        searchInput.placeholder = baseText + currentText;
        ...
        if (!isDeleting && charIndex < currentWord.length) {
            ...
            setTimeout(typeEffect, isDeleting ? delayBetweenWords : typingSpeed);
        }
    }
    typeEffect();
}
...
const sortProductsDropdown = document.getElementById('sort-products-dropdown');
 sortProductsDropdown.addEventListener('change', () => {
        currentPage = 1; 
...
const addToCart = (productId, size, quantity, triggerElement) => {
    ...
    const hasPackageItem = cart.some(item => item.isBundle || item.isCustomPackage);
    const hasSingleItem = cart.some(item => !item.isBundle && !item.isCustomPackage);

    if (cart.length > 0) {

        if (hasPackageItem) {
            showToast('toast_package_in_cart_warning', 'error');
            return;
        }

        if (hasSingleItem && (product.isBundle || product.isCustomPackage)) {
            showToast('toast_single_items_in_cart_warning', 'error');
            return;
        }
    }
...
APP.addPackageToCart = (packageItem) => {
    
    const hasPackageItem = cart.some(item => item.isBundle || item.isCustomPackage);
    const hasSingleItem = cart.some(item => !item.isBundle && !item.isCustomPackage);

    if (hasPackageItem) {
        APP.showToast('toast_package_in_cart_warning', 'error');
        return false; 
    }

    if (hasSingleItem) {
        APP.showToast('toast_single_items_in_cart_warning', 'error');
        return false; 
    }

    packageItem.isCustomPackage = true;
    packageItem.isBundle = true;
    cart.push(packageItem);
    saveCart();
    refreshAllCartViews();
    APP.showToast('Paket Kustom berhasil ditambahkan ke keranjang!', 'success');
    return true; 
};
...
const updateShippingEstimates = () => {
    ...
    const container = document.getElementById('expedition-method');
    if (!container) return;

    const jakartaKeywords = ['jakarta selatan', 'jakarta timur', 'jakarta barat', 'jakarta pusat', 'jakarta utara'];
    const jabodetabekKeywords = ['bogor', 'depok', 'tangerang', 'bekasi', ...jakartaKeywords];
    
    const isJakarta = jakartaKeywords.some(keyword => address.includes(keyword));
    const isJabodetabek = jabodetabekKeywords.some(keyword => address.includes(keyword));

    const availableOptions = expeditionMethods.filter(option => {
        return !option.isJakartaOnly || (option.isJakartaOnly && isJakarta);
    });

    container.innerHTML = '';
    container.className = 'luxury-shipping-hub';

    if (availableOptions.length === 0) {
        container.innerHTML = '<p class="info-message">Tidak ada metode pengiriman tersedia untuk alamat ini.</p>';
        return;
    }

    availableOptions.forEach((option, index) => {
        let estimateText = 'Isi Alamat';
        if (address.trim()) {
            if (option.isInstant) { 
                estimateText = '1 Hari Kerja'; 
            } else {
                estimateText = isJabodetabek ? '1-4 Hari Kerja' : '3-7 Hari Kerja';
            }
        }
        
        const isRecommended = option.id === 'jne';
        const optionHTML = `
            <div class="radio-option-luxury">
                <input type="radio" id="${option.id}" name="expeditionMethod" value="${option.name}" ${index === 0 ? 'checked' : ''} required>
                <label for="${option.id}" class="shipping-card-luxury ${isRecommended ? 'recommended' : ''}">
                    <div class="shipping-card-border"></div>
                    ${isRecommended ? '<div class="recommendation-badge"><i class="fas fa-star"></i> Paling Populer</div>' : ''}
                    <div class="shipping-card-content">
                        <div class="shipping-card-logo"><img src="${option.logo}" alt="${option.name} logo"></div>
                        <div class="shipping-card-details">
                            <span class="shipping-name">${option.name}</span>
                            <span class="shipping-service">${option.service}</span>
                        </div>
                        <div class="shipping-card-estimate">
                            <i class="far fa-clock"></i>
                            <span class="dynamic-estimate">${estimateText}</span> 
                        </div>
                    </div>
    ...
    if (typeof initializeShippingCardShine === 'function') {
        initializeShippingCardShine();
    }
};

function initializeShippingCardShine() {
    const shippingCards = document.querySelectorAll('.shipping-card-luxury');
...
checkoutForm.addEventListener('submit', e => {
    ...
    let orderDetails = `*Order Baru dari Luxuliver Shop*\n\n` +
        ...
        `*Diskon Pembelian:* -${formatRupiah(discount)}\n` + shippingDiscountText + `*Total Pembayaran:* ${formatRupiah(total)}\n\n` + `Terima kasih! Detail biaya total seluruhnya termasuk biaya pengiriman akan diinfokan oleh admin kami.`;
...
whatsappConfirmYesBtn.addEventListener('click', () => {
    if (pendingOrder) {
        const formData = new FormData(checkoutForm);
        const customerName = formData.get('customer-name').split(' ')[0]; 
        ...
        document.getElementById('thank-you-title').textContent = `Terima Kasih, ${customerName}!`;

        openModal(document.getElementById('thank-you-modal'));
...
const thankYouContinueBtn = document.getElementById('thank-you-continue-btn');
const thankYouHistoryBtn = document.getElementById('thank-you-history-btn');
const thankYouModal = document.getElementById('thank-you-modal');

thankYouContinueBtn.addEventListener('click', () => {
    closeModal(thankYouModal);
    showMainContentSection('#koleksi'); 
    window.scrollTo({ top: 0, behavior: 'smooth' });
});

thankYouHistoryBtn.addEventListener('click', () => {
    closeModal(thankYouModal);
    showMainContentSection('#riwayat-pesanan'); 
});
...
const updateFiltersAndRender = (newFilters) => {
    ...
    if (APP.updateLiveFilterUI) APP.updateLiveFilterUI();
    updateFilterUI(); 
};
...
const renderAllProductShowcases = (searchTerm = '') => {
    ...
    let filteredProducts = products.filter(p => !p.isBundle && !p.isReward);
    ...
    if (activeFilters.availability === 'ready') filteredProducts = filteredProducts.filter(p => p.status !== 'preorder');
    if (activeFilters.availability === 'preorder') filteredProducts = filteredProducts.filter(p => p.status === 'preorder');
    ...
    if (activeFilters.promo) filteredProducts = filteredProducts.filter(p => p.isPromo === true);

    const sortValue = sortProductsDropdown.value;
    if (sortValue === 'popularity') filteredProducts.sort((a, b) => (b.reviews?.length || 0) - (a.reviews?.length || 0));
    else if (sortValue === 'price-asc') filteredProducts.sort((a, b) => getPriceBySize(a.basePrice, 'M') - getPriceBySize(b.basePrice, 'M'));
    else if (sortValue === 'price-desc') filteredProducts.sort((a, b) => getPriceBySize(b.basePrice, 'M') - getPriceBySize(a.basePrice, 'M'));
    
    const indexOfLastProduct = currentPage * productsPerPage;
    const indexOfFirstProduct = indexOfLastProduct - productsPerPage;
    const currentProducts = filteredProducts.slice(indexOfFirstProduct, indexOfLastProduct);
    
    renderProducts(currentProducts, productList, noResultsMessage);

    renderPaginationControls(filteredProducts.length);

    const isFilterOrSearchActive = activeFilters.availability !== 'all' || activeFilters.color !== 'all' || activeFilters.design !== 'all' || activeFilters.promo || currentSearchTerm;
...
const calculateCartTotals = () => {
    ...
    const hasPackageItem = cart.some(item => item.isBundle === true || item.isCustomPackage === true);

    if (!hasPackageItem && cart.length > 0) {

        if (totalItems >= 5) {
            productDiscount = subtotal * 0.05;
        }
    }
    ...
    if (cart.length > 0 && addressInput && addressInput.value) {
        ...
        for (const part of [...addressParts].reverse()) {
            if (streetPrefixes.some(prefix => part.startsWith(prefix + '.') || part.startsWith(prefix + ' '))) {
                continue;
            }
            for (const discount of shippingDiscounts) {
                if (new RegExp('\\b' + discount.city.toLowerCase() + '\\b').test(part)) {
            ...
            for (const city of jabodetabekCities) {
                if (new RegExp('\\b' + city + '\\b').test(part)) {
    ...
    return {
        subtotal,
        totalItems,
        totalSingleItems: hasPackageItem ? 0 : totalItems,
...
const renderReviewPagination = (totalReviews) => {
    ...
    prevButton.addEventListener('click', () => changeReviewPage(currentReviewPage - 1));
    paginationContainer.appendChild(prevButton);

    const pageIndicator = document.createElement('span');
    pageIndicator.className = 'page-indicator';
    pageIndicator.textContent = `${currentReviewPage} / ${totalPages}`;
    paginationContainer.appendChild(pageIndicator);

    const nextButton = document.createElement('button');
    nextButton.innerHTML = '<i class="fas fa-chevron-right"></i>';
    nextButton.disabled = currentReviewPage === totalPages;
    nextButton.addEventListener('click', () => changeReviewPage(currentReviewPage + 1));
    paginationContainer.appendChild(nextButton);
};


const changeReviewPage = (pageNumber) => {
...
const renderAllReviews = () => {
    ...
    if (currentDesignFilter !== 'all') {
        filteredReviews = filteredReviews.filter(review => review.productDesign === currentDesignFilter);
    }
    ...
    const reviewsToRender = filteredReviews.slice(indexOfFirstReview, indexOfLastReview);

   
    if (reviewsToRender.length === 0) {
        if (filteredReviews.length > 0) { 
            emptyAllReviewsMessage.textContent = `Tidak ada hasil di halaman ${currentReviewPage}. Coba kembali ke halaman pertama.`;
        } else { 
            emptyAllReviewsMessage.textContent = `Tidak ada ulasan yang cocok dengan filter yang Anda pilih.`;
        }
        emptyAllReviewsMessage.style.display = 'block';
        allReviewsList.innerHTML = '';
    } else {
        emptyAllReviewsMessage.style.display = 'none';
        allReviewsList.innerHTML = reviewsToRender.map(review => {
    ...
    renderReviewPagination(filteredReviews.length);
};
...
const updateResetButtonVisibility = () => {
    if (resetFiltersBtn) {
        ...
        const relevantFilters = {
            ...
            design: activeFilters.design
        };

       
        const isAnyFilterActive = Object.values(relevantFilters).some(v => v !== 'all');

        resetFiltersBtn.style.display = isAnyFilterActive ? 'block' : 'none';
    }
};
...
checkoutBtn.addEventListener('click', () => {
    ...
    cart.forEach(item => {
        if (item.isCustomPackage) return;
...
nextStepBtn.addEventListener('click', () => {
    if (validateStep(currentStep)) {
        ...
        if (currentStep === 2) {
            updateShippingEstimates();
            setTimeout(initializeShippingCardShine, 100); 
        }
        if (currentStep === 3) {
            renderFinalSummary();
        }
...
const initializeReviewFilters = () => {
    ...
    filterContainer.addEventListener('click', (e) => {
        ...
        const ratingBtn = e.target.closest('.rating-filter-btn');
        if (ratingBtn) {
            currentRatingFilter = parseInt(ratingBtn.dataset.rating, 10);
        ...
        const designBtn = e.target.closest('.design-filter-btn');
        if (designBtn) {
            currentDesignFilter = designBtn.dataset.design;
            filterContainer.querySelectorAll('.design-filter-btn').forEach(btn => btn.classList.remove('active'));
...
const updateAuthStateUI = () => {
    ...
    if (isAuthenticated()) {
        ...
        if (sidebarToggleButton) sidebarToggleButton.classList.remove('guest-hidden');
        if (collectionControls) collectionControls.classList.remove('guest-hidden');
        ...
        if (sidebarToggleButton) sidebarToggleButton.classList.add('guest-hidden');
        if (collectionControls) collectionControls.classList.add('guest-hidden');
...
function initializeTour() {
 ...
 const addressInput = document.getElementById('customer-address');
    if (addressInput) {
        addressInput.addEventListener('input', updateShippingEstimates);
    }
...
if (returnConfirmationModal) {
    ...
    if (sessionStorage.getItem('luxuliver_visited')) {
        ...
        if (loadingVideo) {
            loadingVideo.addEventListener('ended', () => {
                sessionStorage.setItem('luxuliver_visited', 'true'); 
                startApp();
            });
...
const sortWrapper = document.querySelector('.sorting-controls-container .custom-select-wrapper');
if (sortWrapper) {
    const trigger = sortWrapper.querySelector('.custom-select-trigger');
    const valueDisplay = sortWrapper.querySelector('#custom-select-value');
    const optionsContainer = sortWrapper.querySelector('.custom-select-options');
    const originalSelect = sortWrapper.querySelector('.original-select');

    if (originalSelect && optionsContainer && optionsContainer.children.length === 0) {
    ...
    if(trigger) {
        trigger.addEventListener('click', (e) => {
            ...
            document.querySelectorAll('.filter-select.open').forEach(w => w.classList.remove('open'));
...
document.addEventListener('click', (e) => {
    if (!e.target.closest('.sorting-controls-container .custom-select-wrapper')) {
        const openWrapper = document.querySelector('.sorting-controls-container .custom-select-wrapper.open');
        if (openWrapper) openWrapper.classList.remove('open');
    }
});
...
if (logoutMenuItem) {
    ...
    APP.attachProductCardListeners = attachProductCardListeners;
    APP.updateFiltersAndRender = updateFiltersAndRender;
